 ...
 * the GD32 HAL flash routines) and the standalone bootloader. Both sides
 * MUST agree on these definitions, so they live in one header.
 *
 * 512 KB flash map (GD32E517RE):
 *
 ...
 *   0x08080000  end
 *
 * The signed manifest trailer occupies the last 2 KB page of each slot and is
 * written once by OTA commit. The mutable per-slot boot state lives in the
 * separate boot-state page so it can change (PENDING -> TRYING -> VALID /
 * INVALID) without rewriting signed data.
 */
...
#define PAPR_SLOT_APP_MAX        (PAPR_SLOT_SIZE - PAPR_FLASH_PAGE) /* 238 KB */
